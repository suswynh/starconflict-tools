...
| 工具 | 覆盖率 | 用法 |
|------|--------|------|
| `rawtex_py.py`（自研） | ~50%（简单TFH） | `python rawtex_py.py dir/ --auto` |
| `tex_StarConflict_tfh_tfd_v2.py`（Noesis插件） | ~70%（fonts ✅, _d/_nm ✅） | 在 Noesis 中拖放 .tfh 文件 |
| `rawtex` (id-daemon) | ~90%（备用） | `RawtexCmd.exe file.dds DXT5` |

**已转换可用**：fonts (70/70)、_d/_nm（简单+压缩）、mapskit/decorative 基本可用。

**卡点**：_s1 (BC5/ATI2非方形)、_s 新版、背景/关卡贴图仍未解决，占压缩TFH中的大部分。
